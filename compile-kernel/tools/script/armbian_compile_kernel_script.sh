    ...
    steps:
      ...
      - name: Initialization environment
        ...
        run: |
          ...
          # 设置系统参数以优化编译环境
          echo "Setting system parameters for optimal compilation..."
      ...
      - name: Create a mapped directory
        ...
        run: |
          ...
          mkdir -p /home/runner/work/_actions/xmyts/amlogic-s9xxx-armbian/main/compile-kernel/
          ln -sf /builder/kernel /home/runner/work/_actions/xmyts/amlogic-s9xxx-armbian/main/compile-kernel/kernel
          ln -sf /builder/output /home/runner/work/_actions/xmyts/amlogic-s9xxx-armbian/main/compile-kernel/output
          echo "status=success" >> ${GITHUB_OUTPUT}

      - name: Compile the kernel [ ${{ inputs.kernel_version }} ]
        uses: xmyts/amlogic-s9xxx-armbian@main
        ...
        with:
          ...
          custom_kernel_path: /builder/kernel
          custom_output_path: /builder/output
          # 调整并行编译参数，避免过度使用资源
          parallel_jobs: $(expr $(nproc) - 1)  # 使用比CPU核心数少1的并行任务
          # 启用增量编译
          clean_mode: false
          # 优化内存使用
          memory_limit: 8g
